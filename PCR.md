...
Or with additional read-only virtual cluster:

```sql
WITH READ VIRTUAL CLUSTER;
```
...
Stop replication process of the latest update:

```sql
ALTER VIRTUAL CLUSTER main COMPLETE REPLICATION TO LATEST;
```

Or alternatively as of a timestamp:
